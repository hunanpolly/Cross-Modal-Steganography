...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if 'ix_to_word' in self.info:
            ...
            for idx, word in self.ix_to_word.items():
    ...
    def get_batch(self, split, secret_size, batch_size=None):
        ...
        for i in range(batch_size):
            ...
            sec_mes = np.random.binomial(1, 0.5, secret_size)
            sec_mes = torch.from_numpy(sec_mes).float()
            sec_mes_batch.append(sec_mes)
        ...
        data['sec_mes'] = np.stack(sum([[_]*seq_per_img for _ in sec_mes_batch], []))
        # merge att_feats
